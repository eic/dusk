...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    double nx                     = std::stod(args.at(1));
    double ny                     = std::stod(args.at(2));
    double nz                     = std::stod(args.at(3));
    double d                      = std::stod(args.at(4));
    std::string inputFile         = args.at(5);
    std::string outputFile =
        (argc >= 7) ? args.at(6) : (fs::path(inputFile).stem().string() + "_cut.stp");
    ...
    // 1. Load STEP
    // ------------------------------------------------------------------
    TopoDS_Shape shape = loadStep(inputFile);

    // ------------------------------------------------------------------
    // 2. Build half-space solid
    //    dawncut convention: plane equation is  ax + by + cz + d = 0
    //    after normalisation, the plane origin is p0 = -(n)*d
